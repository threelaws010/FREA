...
def store_file(file_path: str, vectorstore: Any, embeddings_impl: Any):
    ...
    if vectorstore is None:
        vectorstore = FAISS.from_documents(docs, embeddings_impl)
    else:
        vectorstore.add_documents(docs)

    print(f"✅ Inserted into FAISS: {file_path}")
    return vectorstore

def load_vectorstore(embeddings_impl: Any):
    dir_path = index_dir_for_embedding()
    faiss_idx = dir_path / "index.faiss"
    pkl_idx = dir_path / "index.pkl"

    if faiss_idx.exists() and pkl_idx.exists():
        print(f"✅ Found FAISS index → {dir_path}")
        # WARNING: allow_dangerous_deserialization=True for older LC pickles
        return FAISS.load_local(
            dir_path,
            embeddings=embeddings_impl,
            allow_dangerous_deserialization=True
        )
    print("⚠️ No FAISS index found for this embedding yet.")
    return None
...
def query_loop():
    ...
    vectorstore = load_vectorstore(embeddings_impl)
    if not vectorstore:
        print("⚠️ No vectorstore to query for this embedding.")
        return
