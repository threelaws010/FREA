...
--------------------------------
Processes JPG images in INPUT_DIR using a *local* model served by LM Studio's
OpenAI-compatible API. Produces structured text outputs similar to the original
script you shared, but without calling OpenAI's hosted API.
...
def main():
    ...
    if not images:
        print(f"ℹ️ No .jpg files found in {INPUT_DIR}")
        return
    ...
    for file_name in images:
        ...
        output_name = f"{Path(MODEL).name}_{TOKENS}_{base}.txt".replace("/", "_")
        output_path = os.path.join(INPUT_DIR, output_name)

        if os.path.exists(output_path) and not OVERRIDE:
            print(f"↩️  Skipping (exists): {file_name}")
            continue

        print(f"🖼️  Processing: {file_name} with model '{MODEL}' via {BASE_URL}")
        try:
